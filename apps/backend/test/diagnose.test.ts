...
const URL = 'https://plantcare-api.test/api/diagnose';
...
const HIGH_CONF = JSON.stringify({
  ...
  confidence: 85,
  severity: 'medium',
  fix_steps: ['Isolate the plant.', 'Rinse leaves.'],
  alternatives: [],
});
...
describe('POST /api/diagnose', () => {
  it('rejects missing X-Device-Id header', async () => {
    ...
    const body = (await res.json()) as ApiResult<DiagnoseResponse>;
  ...
  it('returns success on free-tier high-confidence response', async () => {
    ...
    if (body.ok && body.kind === 'success') {
      expect(body.data.disease_slug).toBe('spider_mites');
      expect(body.data.severity).toBe('medium');
      expect(body.data.fix_steps).toEqual(['Isolate the plant.', 'Rinse leaves.']);
  ...
  it('returns degraded fallback when both free and paid OpenRouter calls return 500', async () => {
    ...
    if (body.ok && body.kind === 'success') {
      ...
      expect(body.data.disease_slug).toBe('unknown');
      expect(body.data.disease_label).toBe('Unknown');
      expect(body.data.confidence).toBe(0);
      // 'medium' on no-info fallback — see router.ts diagnose fallback comment.
      expect(body.data.severity).toBe('medium');
      expect(body.data.fix_steps).toEqual([]);
